...
const HolidayYearPlans: React.FC = () => {
    const [holidayPlans, setHolidayPlans] = useState<IHoliday[]>([])
    ...
    useEffect(() => {
      const fetchHolidayPlans = async () => {
          ...
          if (token) {
              try {
                  const response = await fetch('http://127.0.0.1:8000/api/list_holiday_approved_requests_plans', {
                      method: 'GET',
                      headers: {
