    ...
    const handleSubmit = async () => {
        if (token) {
            try {
                ...
                const response = await fetch("http://127.0.0.1:8000/api/create_holiday_plan_request", {
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Token ${token}`,
                ...
                if (!response.ok) {
                    ...
                    toast.error('Wystąpił błąd podczas przetwarzania Twojego planu.')
                    throw new Error(`HTTP error! Status: ${response.status}, Error: ${errorMessage}`);
                }

                sendNotifications({ id: uuid(), label: `Nowy plan urlopowy (${selectedHolidayType}) dla ${first_name} ${last_name} | (${handleGetCurrentTime()})` }, token)

                toast.success('Pomyślnie dodano plan urlopowy.')

                clearInputs();

            } catch (error) {
                toast.error('Wystąpił błąd podczas wysyłania planu urlopowego.')
            }
        }
    };
