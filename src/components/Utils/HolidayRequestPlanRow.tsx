...
const HolidayRequestPlanRow: React.FC<IListRow> = ({ userInfo, requestInfo }) => {
    ...
    const handleApprove = async (id: string) => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/approve_holiday_plan_request/${id}`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Token ${token}`,
            ...
            if (response.ok) {
                setIsApproved(true)
                console.log('Pomyslnie zatwierdzono plan')

                notify('accept', 'Pomyslnie zatwierdzono plan')

            } else {
                console.error('Failed to approve holiday plan');
            }
        } catch (error) {
    ...
    const handleReject = async (id: string) => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/reject_holiday_plan_request/${id}`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Token ${token}`,
            ...
            if (response.ok) {
                setIsApproved(false)
                console.log('Pomyslnie odrzucono plan')

                notify('reject', 'Pomyslnie odrzucono plan')

            } else {
                console.error('Failed to reject holiday plan');
            }
        } catch (error) {
            console.error('Error while processing the plan', error);
        }
    };
...
export default HolidayRequestPlanRow